...
const MyAppointments = () => {
  ...
  return (
    <div className='mx-2 mb-8'>
      <p className='pb-3 mt-5 text-lg sm:text-2xl font-medium text-white text-center'>My appointments</p>
      ...
      <div className='sm:flex sm:flex-row sm:flex-wrap sm:gap-5 justify-center'>
        {appointments.map((item, index) => (
          <div key={index} className='grid grid-cols-[1fr_2fr] gap-5 sm:flex sm:gap-6 py-4 border-b bg-white rounded-lg sm:w-[40%]'>
            ...
            <div></div>
            <div className='flex flex-col gap-2 justify-center text-sm text-center sm:p-4'>
              {!item.cancelled && !item.payment && !item.isCompleted  && <button onClick={() => appointmentRazorpay(item._id)} className='text-[#696969] sm:min-w-48 py-2 border rounded hover:bg-gray-100 hover:text-white transition-all duration-300 flex items-center justify-center'><img className='max-w-20 max-h-5' src={assets.razorpay_logo} alt="" /></button>}
